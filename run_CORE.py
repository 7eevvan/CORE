...
if __name__ == "__main__":
    ...
    #adj_dict = read_clq_graph('graphs/brock400_2.clq')
    ...
    best_clique, best_time = CORE(
        ...
        gamma = 0.5**10,            # density threshold for quasi-clique
        no_improve_limit = 10000,   # max iterations without improvement before stopping current k-size search
        max_iterations = 1e6,       # max global iterations across all k-sizes
        ...
        max_region_size = 50,       # maximum number of nodes to include in region
        seed = 1,                   # random seed 
    )
